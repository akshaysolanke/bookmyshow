...
import "./MultiMuvieCard.css"
import { MdChevronRight } from "react-icons/md";
import MultiCarousel from 'react-multi-carousel';
import 'react-multi-carousel/lib/styles.css';

const MultiMovieCard = ({title,isShowAll=false,srcData=[]}) => {
    const responsive = {
        superLargeDesktop : {
  ...
  return (
    <div className='a_movies_carousel my-5'>
            ...
            {(title || isShowAll) && <div className='d-flex justify-content-between align-items-center my-2 mx-1'>
                <h4 className='a_h20 fw-bold Ps-3'>{title}</h4>
                ...
                <MultiCarousel responsive={responsive}>
                    {srcData.map((value,index) => {
                        return <div  className='a_carousel_img_box a_cursor_pointer'>
                            <img src={value} alt="" />
                        </div>
