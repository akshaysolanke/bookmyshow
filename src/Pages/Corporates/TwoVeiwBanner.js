...
const data = [
  ...
  return data.map(({btnText,desc,heading,imgPath,title1},index) => {
    return(
        <div key={index} style={{borderBottom:"5px solid #808080"}} className={`row ${index % 2 === 0 ? "flex-row-reverse" : ""}`}>
            <div className="col-12 col-md-6 p-0">
                <img src={`Images/corporates/${imgPath}`} width={"100%"} alt="" />
            </div>
