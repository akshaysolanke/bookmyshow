...
const Description = () => {
    ...
    let data = [
        ...
        {
            description : "Don't wait anymore and book your movie tickets from BookMyShow today at the best price! Your access to your favourite movie in Mumbai is only a click away!"
        },
    ];

    if(pathname==="/listyourshow") {
        data = [
            {
                title:"Privacy Note",
                description : (
                    <span>
                        ...
                        <Link  to="/privacy"
                            className="a_text_red_light_color text-decoration-none">
  ...
  return (
  <div className='a_Background py-2'>
    <section className='container my-3 a_Background'>
    {data.map((value,index)=>{
        return(
            <div key={index}>
                <h6 className='text-muted a_h14'>{value.title}</h6>
                <p className='a_h12 fw-light text-muted mt-2'>
                    {
                        value.description
                    }
                </p>
